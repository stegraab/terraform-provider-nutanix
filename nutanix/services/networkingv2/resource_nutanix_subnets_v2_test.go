...
func TestAccV2NutanixSubnetResource_UpdateDhcpOptionsWithoutIpConfigChange(t *testing.T) {
	r := acctest.RandInt()
	name := fmt.Sprintf("tf-test-subnet-%d", r)
	desc := "test subnet description"
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testSubnetV2ConfigWithIPPoolAndDHCP(name, desc, "8.8.8.8"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceNameSubnet, "name", name),
					resource.TestCheckResourceAttr(resourceNameSubnet, "dhcp_options.0.domain_name_servers.0.ipv4.0.value", "8.8.8.8"),
			...
			{
				Config: testSubnetV2ConfigWithIPPoolAndDHCP("updated-"+name, "updated-"+desc, "1.1.1.1"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceNameSubnet, "name", "updated-"+name),
					resource.TestCheckResourceAttr(resourceNameSubnet, "dhcp_options.0.domain_name_servers.0.ipv4.0.value", "1.1.1.1"),
					resource.TestCheckResourceAttr(resourceNameSubnet, "ip_config.0.ipv4.0.ip_subnet.0.ip.0.value", "192.168.0.0"),
					resource.TestCheckResourceAttr(resourceNameSubnet, "ip_config.0.ipv4.0.ip_subnet.0.prefix_length", "24"),
				),
			},
...
func testSubnetV2ConfigWithIPPoolAndDHCP(name, desc, dns string) string {
	return fmt.Sprintf(`
		data "nutanix_clusters_v2" "clusters" {}
